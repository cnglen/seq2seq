    ...
    def _build(self, features, labels, params):
        ...
        if self.mode == tf.contrib.learn.ModeKeys.INFER:
            ...
            if self.mode == tf.contrib.learn.ModeKeys.TRAIN:
                tf.summary.scalar("train_loss", tf.to_float(loss))
                tf.summary.histogram("train_losses", tf.to_float(losses))
            else:
                tf.summary.scalar("validation_loss", tf.to_float(loss))
                tf.summary.histogram("validation_losses", tf.to_float(losses))
